use crate::solver_base::{CellIndex, CellIndices, FlatIndex, get_quad_offset, Indices, QuadrantIndex, QuadrantIndices};

const fn generate_row_mask(c: CellIndex) -> u128 {
    const ROW_MASK: u128 = (1u128 << (0 * 9)) | (1 << (1 * 9)) | (1 << (2 * 9)) | (1 << (3 * 9)) | (1 << (4 * 9)) | (1 << (5 * 9)) | (1 << (6 * 9)) | (1 << (7 * 9)) | (1 << (8 * 9));
    (ROW_MASK << (c.get()))
}
...
fn generate_quadrant_mask1(cell_indices: CellIndices) -> u128 {
    const QUADRANT_MASK: u64 = 0b111 | (0b111 << 9) | (0b111 << 2 * 9);
    let quadrant_offset = get_quad_offset(QuadrantIndices::from(cell_indices));
    ((QUADRANT_MASK as u128) << quadrant_offset.get())
}

const fn generate_quadrant_mask2(cell_indices: CellIndices) -> u128 {
    const QUADRANT_MASK: u64 = 0b111 | (0b111 << 9) | (0b111 << 2 * 9);
    let quadrant_indices = Indices {
        row: QuadrantIndex::checked_new(cell_indices.row.get() / 3),
        column: QuadrantIndex::checked_new(cell_indices.column.get() / 3)
    };

    let quadrant_start = Indices {
        row: CellIndex::checked_new(quadrant_indices.row.get() * 3),
        column: CellIndex::checked_new(quadrant_indices.column.get() * 3),
    };
    let quadrant_offset = FlatIndex::checked_new(quadrant_start.row.get() * 9 + quadrant_start.column.get());
    (QUADRANT_MASK as u128) << quadrant_offset.get()
}
...
fn test_row_masks() {
    ...
    for c in 0..9 {
        ...
        let m1 = crate::solver_full_loop::generate_row_mask1(c) & check_mask;
        let m2 = crate::solver_full_loop::generate_row_mask2(c) & check_mask;
...
const fn generate_mask_const(cell_indices: CellIndices) -> u128 {
    generate_column_mask(cell_indices.row) | generate_row_mask(cell_indices.column) | generate_quadrant_mask2(cell_indices)
}
